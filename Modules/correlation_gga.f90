!
!-----------------------------------------------------------------------
SUBROUTINE perdew86( length, rho, grho, sc, v1c, v2c )
  !-----------------------------------------------------------------------
  ...
  REAL(DP), INTENT(OUT), DIMENSION(length) :: sc, v1c, v2c
  !
  ! ... local variables
  !
  INTEGER :: ir
  ...
  DO ir = 1, length
     ...
     !
     phi = 0.192_DP * pci / cn * SQRT(grho(ir)) * rho(ir)**(-7._DP/6._DP)
     ! SdG: in the original paper 1.745*0.11=0.19195 is used
     ephi = EXP( -phi )
     !
     sc(ir)  = grho(ir) / rho43 * cn * ephi
     v1c(ir) = sc(ir) * ( (1._DP+phi) * dcn / cn - ((4._DP/3._DP)-(7._DP/ &
                     6._DP)*phi) / rho(ir) )
     v2c(ir) = cn * ephi / rho43 * (2._DP - phi)
     !
  ENDDO
  !
  RETURN
...
!
!-----------------------------------------------------------------------
SUBROUTINE ggac( length, rho, grho, sc, v1c, v2c )
  !-----------------------------------------------------------------------
  ...
  rs = pi34 / rho**third
  !
  DO ir = 1, length   !^^^ PROVISIONAL
     CALL pw( rs(ir), 1, ec(ir), vc(ir) ) 
  ENDDO
  !
  DO ir = 1, length
     !
     ...
     kf = xkf / rs1
     ks = xks * SQRT(kf)
     t = SQRT(grho(ir)) / (2._DP * ks * rho(ir))
     !
     expe = EXP( - 2._DP * al * ec(ir) / (be * be) )
     ...
     dh0 = be * t * t / s1 * ( - 7._DP / 3._DP * xy - qy * (af * bf / &
                               be-7._DP / 3._DP) )
     ddh0 = be / (2._DP * ks * ks * rho(ir)) * (xy - qy) / s1
     !
     ...
     dh1 = - third * ( h1 * (7._DP + 8._DP * ee) + nu * t * t * EXP(ee) &
                       * dcn )
     ddh1 = 2._DP * h1 * (1._DP + ee) * rho(ir) / grho(ir)
     !
     sc(ir)  = rho(ir) * (h0 + h1)
     v1c(ir) = h0 + h1 + dh0 + dh1
     v2c(ir) = ddh0 + ddh1
     !
  ENDDO
  !
...
!
!-----------------------------------------------------------------------
SUBROUTINE glyp( length, rho, grho, sc, v1c, v2c )
  !-----------------------------------------------------------------------
  ...
  DO ir = 1, length
     ...
     rhom43 = rhom13**4
     !
     v1c(ir) = - ff * rhom43 / 3.d0 * ( 5.d0 * rhom43 * om * xl + rhom53 * &
                                       dom * xl + rhom53 * om * dxl )
     v2c(ir) = 2.d0 * sc(ir) / grho(ir)
     !
  ENDDO
  !
  RETURN
...
!
!---------------------------------------------------------------
SUBROUTINE pbec( length, rho, grho, iflag, sc, v1c, v2c )
  !---------------------------------------------------------------
  ...
  DO ir = 1, length
     ...
     ddh0 = be(iflag) / (2._DP * ks * ks * rho(ir)) * (xy - qy) / s1
     !
     sc(ir)  = rho(ir) * h0
     v1c(ir) = h0 + dh0
     v2c(ir) = ddh0
     ! q2D
     IF (iflag == 3) THEN
        CALL cpbe2d( rho(ir), grho(ir), sc2D, v1c2D, v2c2D )
        sc(ir)  = sc(ir)  + sc2D
        v1c(ir) = v1c(ir) + v1c2D
        v2c(ir) = v2c(ir) + v2c2D
     ENDIF
...
!
!-----------------------------------------------------------------------
SUBROUTINE perdew86_spin( length, rho, zeta, grho, sc, v1c, v2c )
  !---------------------------------------------------------------------
  !! Perdew gradient correction on correlation: PRB 33, 8822 (1986)
  !! spin-polarized case.
  !
  ...
  REAL(DP), INTENT(IN), DIMENSION(length) :: zeta
  !! the magnetization
  REAL(DP), INTENT(IN), DIMENSION(length) :: grho
  ...
  REAL(DP) :: sc_s, zeta_s, rho_s, grho_s
  !
  !
  DO ir = 1, length
    !
    ...
    dcnb = (p3 + 2._DP * p4 * rs + 3.D4 * p2 * rs2) * drs  
    dcn  = dcna / cnb - cna / (cnb * cnb) * dcnb  
    phi  = 0.192_DP * pci / cn * SQRT(grho_s) * rho_s**(-7._DP/6._DP)  
    !SdG: in the original paper 1.745*0.11=0.19195 is used
    !
    dd  = (2._DP)**third * SQRT( ( (1.0_DP + zeta_s) * 0.5_DP)**(5.0_DP/ &  
         3._DP) + ( (1.0_DP - zeta_s) * 0.5d0)**(5._DP/3._DP) )
    !
    ddd = (2._DP)**(-4.0_DP/3._DP) * 5._DP * ( ((1._DP + zeta_s) &  
         * 0.5_DP)**(2._DP/3._DP) - ((1._DP - zeta_s)*0.5d0)**(2._DP/ &  
         3._DP))/(3._DP*dd)
    !
    ephi = EXP(-phi)
    sc_s = grho_s / rho43 * cn * ephi / dd
    !
    sc(ir) = sc_s
    v1c(ir,1) = sc_s * ( (1._DP + phi) * dcn / cn - ( (4._DP / 3._DP) - &  
         (7._DP/6._DP)*phi)/rho_s) - sc_s * ddd / dd * (1._DP - zeta_s)/rho_s
    v1c(ir,2) = sc_s * ( (1._DP + phi) * dcn / cn - ( (4._DP / 3._DP) - &  
         (7._DP/6._DP)*phi)/rho_s) + sc_s * ddd / dd * (1._DP + zeta_s)/rho_s
    v2c(ir) = cn * ephi / rho43 * (2._DP - phi) / dd
    !
  ENDDO
  !
  !
...
!
!-----------------------------------------------------------------------
SUBROUTINE ggac_spin( length, rho_in, zeta_in, grho_in, sc, v1c, v2c )
  !---------------------------------------------------------------------
  ...
  !
  INTEGER :: ir
  REAL(DP), DIMENSION(length) :: rs_v, ec_v
  ...
  !
  !
  rs_v = pi34 / rho_in**third
  !
  DO ir = 1, length  !^^^ PROVISIONAL
     CALL pw_spin( rs_v(ir), zeta_in(ir), ec_v(ir), vc(ir,:) )
  ENDDO
  !
  DO ir = 1, length
    !
    ...
    af = 2._DP * al / be * (1._DP / (expe-1._DP) )
    bfup = expe * (vc(ir,1) - ec) / fz3
    bfdw = expe * (vc(ir,2) - ec) / fz3
    !
    y = af * t * t
    xy = (1._DP + y) / (1._DP + y + y * y)
    qy = y * y * (2._DP + y) / (1._DP + y + y * y)**2
    ...
    !
    sc(ir)    = rho * (h0 + h1)
    v1c(ir,1) = h0 + h1 + dh0up + dh1 + dh0zup + dh1zup
    v1c(ir,2) = h0 + h1 + dh0up + dh1 + dh0zdw + dh1zdw
    v2c(ir)   = ddh0 + ddh1
    !
  ENDDO
  !
...
!
!-------------------------------------------------------------------
SUBROUTINE pbec_spin( length, rho_in, zeta_in, grho_in, iflag, sc, v1c, v2c )
  !-----------------------------------------------------------------
  ...
  DO ir = 1, length
    ...
    !
    sc(ir)    = rho * h0
    v1c(ir,1) = h0 + dh0up + dh0zup
    v1c(ir,2) = h0 + dh0dw + dh0zdw
    v2c(ir)   = ddh0
    !
  ENDDO
  !
...
!
!------------------------------------------------------------------------
SUBROUTINE lsd_glyp( length, rho_in, grho, grho_ud, sc, v1c, v2c, v2c_ud )
  !----------------------------------------------------------------------
  !! Lee, Yang, Parr: gradient correction part.
  !
  USE kinds, ONLY: DP
  ...
  !! the gradient of the charge squared
  REAL(DP), INTENT(IN), DIMENSION(length) :: grho_ud
  ...
  DO ir = 1, length
    ...
    grhoab = grho_ud(ir)
    !
    sc(ir) = dlaa*grhoaa + dlab*grhoab + dlbb*grhobb
    v1c(ir,1) = dlaaa*grhoaa + dlaba*grhoab + dlbba*grhobb    
    v1c(ir,2) = dlaab*grhoaa + dlabb*grhoab + dlbbb*grhobb    
    v2c(ir,1) = 2._DP*dlaa    
    v2c(ir,2) = 2._DP*dlbb    
    v2c_ud(ir)= dlab
    !
