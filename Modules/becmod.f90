...
!                             - CPU, OpenACC (and OpenMP5) cases are distinguished by type(offload_type)
                      ...
                      calbec_k_omp, calbec_gamma_omp, calbec_nc_omp, calbec_bec_type_omp, &
...
CONTAINS
  ...
  SUBROUTINE calbec_bec_type_omp ( offload, npw, beta, psi, betapsi, nbnd )
    ...
    ELSEIF ( noncolin) THEN
       !
       CALL  calbec_nc_omp ( offload_omp, npw, beta, psi, betapsi%nc, local_nbnd )
       !
    ELSE
       !
       CALL  calbec_k_omp ( offload_omp, npw, beta, psi, betapsi%k, local_nbnd )
       !
    ENDIF
    !
    RETURN
