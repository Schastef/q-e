...
If you want to contribute serious or nontrivial stuff (or even simple and trivial stuff), 
you need to  download and keep updated the \qe{} source code. The project has adopted 
\texttt{git} as version control system. It will easy  for you to keep your source 
code aligned just creating a local \textit{clone} of the official \texttt{git} repo  
which is currently  hosted on GitLab at \url{https://gitlab.com/QEF/q-e}. 
A  mirror is also available on  GitHub at  \url{https://github.com/QEF/q-e}. 
When  your contribution is ready,  and  you have ascertained that it is possible to apply 
it as a patch to the current source you have just to submit it to the code 
maintainers.  
Although it is possible to send the diff file to the maintainers it is way better and preferable 
to submit your code contribution as a  \textit{merge request}  to the QEF/q-e project on  GitLab.
You have thus to:

\begin{itemize}
	\item{Create a fork of the \texttt{ QEF\\q-e} project. 
		( see \url{https://docs.gitlab.com/ee/gitlab-basics/fork-project.html} ) }
	\item{Apply your modifications to the fork.}
	\item{Submit the merge request 
		( see \url{https://docs.gitlab.com/ee/user/project/merge_requests/index.html} }
\end{itemize}

Once the merge request has been submitted it will be reviewed by maintainers and merged to the 
distributed repository. 

An exhaustive and not so long guide to \texttt{git} usage may be found in the
online book \url{https://git-scm.com/book/en/v2}. In any case  it is not necessary to 
become a \texttt{git} geek, it will be more than enough if  one is familiar with the most 
basic operations as for example those listed in this 
\href{https://gitlab.com/gitlab-com/marketing/raw/master/design/print/git-cheatsheet/print-pdf/git-cheatsheet.pdf}{cheat-shit}
...
\subsection {\qe{} repository on gitlab.com} 
\qe{} development adopts  \texttt{git} version control system. If you don't  know anything  
about  \texttt{git} you  can have a look at  this \href{https://git-scm.com/book/en/v2}{ebook}.  

The main public repository of \qe{} is  hosted on GitLab at \url{https://gitlab.com/QEF/q-e} and 
a mirror is also available on GitHub at \url{https://github.com/QEF/q-e}. 

To create a your local repository it is sufficient to use the command 

\begin{verbatim}
  git clone https://gitlab.com/QEF/q-e.git 
\end{verbatim}

The repository consists currently of the only (\texttt{master}) branch containing 
the official development  version. Stable versions are indicated by tags which can be listed with 
the command \texttt{git tag -l}. For production, if possible, one should use a stable 
version. To revert the \qe{} directory to the version labeled with the tag \texttt{<tag>} 
it is sufficient to type  the command \texttt{git checkout <tag>}.       

Very small changes meant mainly for maintenance may be pushed directly to the main repository but 
any  significant modiification should be merged into  the main repository using 
{\em merge requests}. 
