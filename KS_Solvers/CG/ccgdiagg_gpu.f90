...
SUBROUTINE ccgdiagg_gpu( hs_1psi_gpu, s_1psi_gpu, precondition_d, &
  ...
  REAL(DP), DEVICE,   INTENT(IN)    :: precondition_d(npwx*npol), ethr
  ...
  COMPLEX(DP), DEVICE, ALLOCATABLE :: hpsi_d(:), spsi_d(:), g_d(:), cg_d(:)
  COMPLEX(DP), DEVICE, ALLOCATABLE :: scg_d(:), ppsi_d(:), g0_d(:), lagrange_d(:)  
  COMPLEX(DP), ALLOCATABLE :: lagrange(:)
  REAL(DP)          :: gamma, ddot_temp, es_1, es(2), e(nbnd)
...
!  REAL (DP), EXTERNAL :: ddot
  ...
  DO m = 1, nbnd
     ...
     if(m_start.le.m_end) &
          CALL ZGEMV( 'C', kdim, m_end-m_start+1, ONE, psi_d(1,m_start), kdmx, spsi_d, 1, ZERO, lagrange_d(m_start), 1 )
     CALL mp_sum( lagrange_d( 1:m ), inter_bgrp_comm )
     !
     CALL mp_sum( lagrange_d( 1:m ), intra_bgrp_comm )
     !
     lagrange(1:m) = lagrange_d(1:m)
     ...
     iterate: DO iter = 1, maxter
        ...
        !
        lagrange_d(1:m-1) = ZERO
        call divide(inter_bgrp_comm,m-1,m_start,m_end); !write(*,*) m-1,m_start,m_end
        if(m_start.le.m_end) &
             CALL ZGEMV( 'C', kdim, (m -1) , ONE, psi_d, kdmx, scg_d, 1, ZERO, lagrange_d, 1 )
        CALL mp_sum( lagrange_d( 1:m-1 ), inter_bgrp_comm )
        !
        DO j = 1, ( m - 1 )
           !
           !$cuf kernel do(1) <<<*,*>>>
  ...
  !
  ! STORING e in e_d where e_d is a GPU allocated array
  e_d = e
  !
  DEALLOCATE( lagrange )
